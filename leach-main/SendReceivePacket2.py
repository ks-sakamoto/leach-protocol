...
def packet(Sensors, Model, Sender, PacketType, Receiver, srp, rrp, sdp, rdp):
    ...
    for i in range(len(Sender)):
        for j in range(len(Receiver)):
            ...
            if distance > Model['d']:
                Sensors['E'][Sender[i]-1] = Sensors['E'][Sender[i]-1] - \
                    (Model['Etx']*PacketSize + Model['Emp']
                     * PacketSize*pow(distance, 4))
                if Sensors['E'][Sender[i]-1] > 0:
                    sap = sap+1
            else:
                Sensors['E'][Sender[i]-1] = Sensors['E'][Sender[i]-1] - \
                    (Model['Etx']*PacketSize + Model['Efs']
                     * PacketSize*pow(distance, 2))
                if Sensors['E'][Sender[i]-1] > 0:
                    sap = sap+1

    for i in range(len(Receiver)):
        Sensors['E'][Receiver[i]-1] = Sensors['E'][Receiver[i]-1] - \
