...
distoCH = []
MCH = []
xd, yd, G, df, types, E, ids, distosink, distoCH, MCH = ConfigSensors2.sensor(
    Area, Model, X, Y, xd, yd, G, df, types, E, ids, distosink, distoCH, MCH)
Sensors = {'xd': xd, 'yd': yd, 'G': G, 'df': df, 'types': types, 'E': E,
           'ids': ids, 'distosink': distosink, 'distoCH': distoCH, 'MCH': MCH}
d = 0
...
Sensors, srp, rrp, sdp, rdp = SendReceivePacket2.packet(
    Sensors, Model, Sender, 'Hello', Receiver, srp, rrp, sdp, rdp)
...
for i in range(1):
    ...
    for k in range(len(TotalCH)):
        ...
        print(type(Sensors))
        print('-----------------')
        Receiver = FindReceiver2.find(
            Sensors, Model, Sender, Receiver, senderRR)
        print(Receiver)
        Sensors = SendReceivePacket2.packet(
            Sensors, Model, Sender, 'Hello', Receiver, srp, rrp, sdp, rdp)
        Sensors = Sensors[0]
        print('====================')
        print(type(Sensors))
    ...
    for j in range(n):
        XL = []
        YL = []
        if Sensors['types'][j] == 'N' and Sensors['distoCH'][j] < Sensors['distosink'][j] and Sensors['E'][j] > 0:
    ...
    for j in range(1):
        ...
        for k in range(len(TotalCH)):
            ...
            Sender = []
            Sender = findsender2.find(Sensors, Model, Receiver, Sender)
            Sensors = SendReceivePacket2.packet(
                Sensors, Model, Sender, 'Hello', Receiver, srp, rrp, sdp, rdp)
            Sensors = Sensors[0]
